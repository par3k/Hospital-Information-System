...
def homepage():
    ...
    return render_template('hrAdmin/index.html', user=user,notice=notice)

@bp_hr.route('/hr/post',methods=['GET', 'POST'])
def post():
    user = MedicalStaff.query.filter(MedicalStaff.StaffID == session["user_id"]).first()
...
@bp_hr.route('/hr/profile',methods=['GET', 'POST'])
def profile():
    if request.method == 'POST':
...
@bp_hr.route('/hr/accountManage', methods=['GET', 'POST'])
def account():
    if request.method == "POST":
        print(request.form)
        ...
        staffInfo = MedicalStaff.query.all()
        user = MedicalStaff.query.filter(MedicalStaff.StaffID == session["user_id"]).first()
        return redirect(url_for('hr.account', staffInfo=staffInfo, user=user))
        #return redirect(url_for('hr.account'))
