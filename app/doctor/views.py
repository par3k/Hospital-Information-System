...
import datetime
from ..model import Schedule, Reservation, Patient, Hospitalization, MedicalStaff, Medicine, db, Prescription, \
    Prescription_Detail, Hspapply, Nursing, Post


@bp_doctor.route('/doctor/homepage')
def homepage():
    ...
    if user.position=='Leader':
        return render_template('doctor/index_leader.html', reservations=reservations, user=user, notice=notice)
    else:
        return render_template('doctor/index.html', reservations=reservations, user=user, notice=notice)


@bp_doctor.route('/doctor/homepageLeader')
def homepageleader():
    dt = datetime.datetime.now().strftime("%Y-%m-%d")
    reservations = Reservation.query.filter(Reservation.doctor_ID == session["user_id"],
                                            Reservation.reservationDate == dt).all()
    user = MedicalStaff.query.filter(MedicalStaff.StaffID == session["user_id"]).first()
    return render_template('doctor/index_leader.html', reservations=reservations, user=user)


@bp_doctor.route('/doctor/profile', methods=['GET', 'POST'])
def profile():
...
@bp_doctor.route('/doctor/mySchedule')
def myschedule():
    dt = datetime.datetime.now().strftime("%Y-%m-%d")
    ...
    for schedule in scheduleInfo :
        print(schedule)
    return render_template('doctor/schedule.html', scheduleInfo=scheduleInfo, user=user)


@bp_doctor.route('/doctor/schedule')
def schedule():
    user = MedicalStaff.query.filter(MedicalStaff.StaffID == session["user_id"]).first()
    ...
    return render_template('doctor/schedule_leader.html', dateInfo=dateInfo, user=user, doctorlist=doctorlist)


@bp_doctor.route('/doctor/scheduling')
def scheduling():
    return redirect(url_for({{'doctor.schedule'}}))


@bp_doctor.route('/doctor/reservationCheck', methods=['GET', 'POST'])
def reservationcheck():
    ...
    user = MedicalStaff.query.filter(MedicalStaff.StaffID == session["user_id"]).first()
    return render_template('doctor/reservationChecklist.html',
                           reservationCheck=reservationCheck, medicineInfo=medicineInfo, user=user)


@bp_doctor.route('/doctor/checkcondition', methods=['GET', 'POST'])
def checkcondition():
...
@bp_doctor.route('/doctor/inpatientList')
def inpatientlist():
    inpatientList = Hospitalization.query.filter_by(doctor_ID=session["user_id"]).all()
    medicineInfo = Medicine.query.all()
    user = MedicalStaff.query.filter(MedicalStaff.StaffID == session["user_id"]).first()
    return render_template('doctor/inpatientList.html',
                           inpatientList=inpatientList, medicineInfo=medicineInfo, user=user)


@bp_doctor.route('/doctor/reservationCheck/diagnose', methods=['GET', 'POST'])
def diagnose():
    print(request.form)
...
@bp_doctor.route('/doctor/reservationCheck/inpatient', methods=['GET', 'POST'])
def inpatient():
    if request.method == "POST":
    ...
    # return render_template('doctor/reservationChecklist.html',
    #                        reservationCheck=reservationCheck, medicineInfo=medicineInfo, user=user)
    return redirect(url_for('doctor.reservationcheck'))


@bp_doctor.route('/doctor/reservationCheck/prescribe', methods=['GET', 'POST'])
def prescribe():
    data = request.get_json()
    # print('here')
    # print(request.get_json())
...
@bp_doctor.route('/doctor/reservationCheck/searchdate', methods=['POST'])
def searchdate():
...
@bp_doctor.route('/doctor/reservationCheck/addschdule', methods=['POST'])
def addschedule():
    data = request.get_json()
    print(request.get_json())
